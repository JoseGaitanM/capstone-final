...
from dags.spark_jobs.create_snapshoots_job import getSubscriptions,getMaxDateRegisters
...
def test_registers_dir(test_dir,test_registers_1,test_registers_2,test_registers_3):
    ...
    test_registers_2.coalesce(1).write.mode('overwrite').format('json').save(temp_dir+'/date=2023-04-08')
    test_registers_3.coalesce(1).write.mode('overwrite').format('json').save(temp_dir+'/date=2023-04-08')

    return temp_dir


def test_getMaxDateRegisters(spark_session, test_registers_dir, data_schema_retrieve_registers, test_registers_1, test_registers_2, test_registers_3):
    dateData, registers = getMaxDateRegisters(test_registers_dir,spark_session)

    assert dateData == datetime.strptime('2023-04-08', '%Y-%m-%d').date()
    assert registers.schema == data_schema_retrieve_registers
    

    

def test_getSubscriptions(spark_session, test_subscriptions_dir):
